...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'project_id', 'user_id', 'date', 'total_share', 'have_to_pay_amount', 'status'];
    }
    ...
    if (search_key) {
        query.where = {
            ...
            [Op.or]: [
                ...
                { total_share: { [Op.like]: `%${search_key}%` } },
                { have_to_pay_amount: { [Op.like]: `%${search_key}%` } },
                { paid: { [Op.like]: `%${search_key}%` } },
