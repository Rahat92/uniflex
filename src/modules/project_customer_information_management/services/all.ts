...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'project_customer_id', 'all_fields_from_hard_copy', 'status'];
    }
    ...
    if (search_key) {
        query.where = {
            ...query.where,
            [Op.or]: [
                { project_customer_id: { [Op.like]: `%${search_key}%` } },
                { all_fields_from_hard_copy: { [Op.like]: `%${search_key}%` } },
    ...
    try {
        let data = await (fastify_instance as anyObject).paginate(
            req,
            models.ProjectCustomerInformationModel,
            paginate,
            query,
        );
