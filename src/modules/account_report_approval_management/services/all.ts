...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'date', 'openning_balance', 'closing_balance', 'total_income', 'total_expence', 'approved_by', 'is_approved', 'not_approved_comment', 'approved_comment', 'status'];
    }
    ...
    if (search_key) {
        query.where = {
            ...
            [Op.or]: [
                ...
                { openning_balance: { [Op.like]: `%${search_key}%` } },
                { closing_balance: { [Op.like]: `%${search_key}%` } },
                { total_income: { [Op.like]: `%${search_key}%` } },
                { total_expence: { [Op.like]: `%${search_key}%` } },
                { approved_by: { [Op.like]: `%${search_key}%` } },
                { is_approved: { [Op.like]: `%${search_key}%` } },
                { not_approved_comment: { [Op.like]: `%${search_key}%` } },
                { approved_comment: { [Op.like]: `%${search_key}%` } },
    ...
    try {
        let data = await (fastify_instance as anyObject).paginate(
            req,
            models.AccountReportApprovalModel,
            paginate,
            query,
        );
        return response(200, 'data fetched', data);
    } catch (error: any) {
        let openning_balance = await error_trace(models, error, req.url, req.query);
