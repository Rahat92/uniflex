...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'project_id','user_id', 'status', 'date', 'time', 'is_complete', 'comments', 'present_time', 'leave_time'];
    }
    ...
    if (search_key) {
        query.where = {
            ...query.where,
            [Op.or]: [
                { is_complete: { [Op.like]: `%${search_key}%` } },
                { comments: { [Op.like]: `%${search_key}%` } },
                { project_id: { [Op.like]: `%${search_key}%` } },
                { user_Id: { [Op.like]: `%${search_key}%` } },
                { time: { [Op.like]: `%${search_key}%` } },
                { date: { [Op.like]: `%${search_key}%` } },
                { present_time: { [Op.like]: `%${search_key}%` } },
                { leave_time: { [Op.like]: `%${search_key}%` } },
    ...
    try {
        let data = await (fastify_instance as anyObject).paginate(
            req,
            models.ProjectVisitHistoryModel,
            paginate,
            query,
        );
