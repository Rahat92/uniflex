...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'account_report_approval_id', 'link', 'status'];
    }
    ...
    if (search_key) {
        query.where = {
            ...query.where,
            [Op.or]: [
                { account_report_approval_id: { [Op.like]: `%${search_key}%` } },
                { link: { [Op.like]: `%${search_key}%` } },
    ...
    try {
        let data = await (fastify_instance as anyObject).paginate(
            req,
            models.AccountReportApprovalDocModel,
            paginate,
            query,
        );
